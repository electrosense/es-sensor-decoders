...
void print_usage()
{
  cout << "LTE CellSearch v" << MAJOR_VERSION << "." << MINOR_VERSION << "." << PATCH_LEVEL << " (" << BUILD_TYPE << ") help screen" << endl
       << endl;
  ...
  cout << "      directory where capbuf_XXXX.it files are located" << endl
       << endl;
  ...
  cout << "to freqency fc*c so that its true frequency shall be fc. Default: 1.0" << endl
       << endl;
  cout << "'ppm' is the remaining frequency error of the crystal. Default: 10" << endl;
  cout << "" << endl;
...
void dedup(
    const vector<list<Cell> > &detected_cells, list<Cell> &cells_final)
{
  cells_final.clear();
...
static ssize_t recvIQSamples(int fd, uint64_t *centerFrequency, uint32_t *gain, uint32_t *nSamples, int8_t *iq)
{
  struct Header hdr;
  ssize_t ret = recv(fd, &hdr, 8 + 4 + 4, MSG_WAITALL);
  if (ret != sizeof hdr)
    return -1;
  *centerFrequency = le64toh(hdr.centerFrequency);
  *gain = le32toh(hdr.gain);
  *nSamples = le32toh(hdr.nSamples);
  ret=recv(fd, iq, *nSamples, MSG_WAITALL);
  if (ret!=*nSamples)
    return -1;
  return ret;
}
...
int decoder_output(char* ID, int fd, struct sockaddr_in *servaddr, const char* msg) {
  char* buffer;
  buffer = (char*)malloc(strlen(msg)+2);
  buffer[0]=0;
  strcat(buffer, ID);
  strcat(buffer, msg);
  sendto(fd, (const char *)buffer, strlen(buffer), MSG_CONFIRM,
    ...
    char *const argv[])
{
  ...
  /* Input socket from es_sensor */
  struct sockaddr_un addr;
  uint64_t centerFrequency;
  uint32_t gain;
  uint32_t nSamples;

  // open socket to read data from socket
  ...
  // Read a first packet to obtain frequency et al
  ssize_t ret = recvIQSamples(fd, &centerFrequency, &gain, &nSamples, iq);
  if (ret < 0)
  {
    error(EXIT_FAILURE, errno, "Socket read failed");
  }

  // Get search parameters from user
  //parse_commandline(argc, argv, freq_start, freq_end, ppm, correction, save_cap,
  //  use_recorded_data, data_dir, device_index);

  use_recorded_data = true;
  double fs_programmed = 1920000;
  freq_start = centerFrequency;
  freq_end = centerFrequency;
  ppm = 10.0;
  correction = 1;
  verbosity = 2;
  ...
  for (uint16 fci = 0; fci < n_fc; fci++)
  {
    ...
    decoder_output(STATUS, output_socket, &servaddr, "Examining center frequency");
    // Fill capture buffer
    cvec capbuf;
    double fc_programmed = fc_requested;
    ...
    int caplength=153600;
    capbuf_raw.reserve(caplength*2);

    // Send message: calculating

    if (verbosity >= 1)
    {
      cout << "Pre-extracting" << fc_requested / 1e6 << " MHz ..." << endl;
    }
    decoder_output(STATUS, output_socket, &servaddr, "Stabilising AGC...");
    // Throw away 1.5s of data to let AGC stabilise at 1.92Msps
    for (int i=0;i<12;i++) {
      ret = recvIQSamples(fd, &centerFrequency, &gain, &nSamples, iq);
      if (ret < 0) {
        error(EXIT_FAILURE, errno, "Socket read failed");
      }
    }
    // Send message: calculating
    int downloaded=0;
    while (downloaded<caplength*2) {
      ret = recvIQSamples(fd, &centerFrequency, &gain, &nSamples, iq);
      if (ret < 0) {
        error(EXIT_FAILURE, errno, "Socket read failed");
      }
      for (int idx=0; idx<ret; idx++) {
        capbuf_raw[downloaded]=iq[idx];
        downloaded++;
        if (downloaded==caplength*2)
          break;
      }
    }
...
#define DS_COMB_ARM 2
    ...
    decoder_output(STATUS, output_socket, &servaddr, "Calculating PSS correlations...");
    ...
    decoder_output(STATUS, output_socket, &servaddr, "Searching for and examining correlation peaks...");
...
#define THRESH2_N_SIGMA 3
      ...
      decoder_output(STATUS, output_socket, &servaddr, "Cell detected!");
      ++iterator;
    }
  }
  ...
  if (cells_final.size() == 0)
  {
    ...
    decoder_output(RESULT, output_socket, &servaddr, "No LTE cells were found.");
  }
  else
  {
    ...
    {
      // Dirty change to limit WebRTC data packet size (a sole JSON with all cells is too big)
      json << "{ \"Cells\": [";
      // Use a stringstream to avoid polluting the iostream settings of cout.
      stringstream ss;
      ss << setw(3) << (*it).n_id_cell();
      json << "{\"CID\": \"" << setw(3) << (*it).n_id_cell()<<"\", ";
      ss << setw(2) << (*it).n_ports;
      json << "\"NumberAntennaPorts\": \"" << setw(2) << (*it).n_ports<<"\", ";
      ss << " " << setw(6) << setprecision(5) << (*it).fc_requested / 1e6 << "M";
      json << "\"CarrierFrequency\": \"" << (*it).fc_requested <<"\", ";
      ss << " " << freq_formatter((*it).freq_superfine);
      json << "\"FrequencyOffsetHz\": \"" << freq_formatter((*it).freq_superfine) <<"\", ";
      ss << " " << setw(5) << setprecision(3) << db10((*it).pss_pow);
      json << "\"RxPowerdB\": \"" << setw(5) << setprecision(3) << db10((*it).pss_pow) <<"\", ";
      ss << " " << (((*it).cp_type == cp_type_t::NORMAL) ? "N" : (((*it).cp_type == cp_type_t::UNKNOWN) ? "U" : "E"));
      json << "\"CPType\": \"" << 
        (((*it).cp_type == cp_type_t::NORMAL) ? "N" : (((*it).cp_type == cp_type_t::UNKNOWN) ? "U" : "E")) <<"\", ";
      ss << " " << setw(3) << (*it).n_rb_dl;
      json << "\"nRB\": \"" << setw(3) << (*it).n_rb_dl <<"\", ";
      ss << " " << (((*it).phich_duration == phich_duration_t::NORMAL) ? "N" : (((*it).phich_duration == phich_duration_t::UNKNOWN) ? "U" : "E"));
      json << "\"PHICHDuration\": \"" << 
        (((*it).phich_duration == phich_duration_t::NORMAL) ? "N" : (((*it).phich_duration == phich_duration_t::UNKNOWN) ? "U" : "E")) <<
        "\", ";
      ...
      json << setprecision(20) << correction_new << "\"} ";
      ...
      decoder_output(RESULT, output_socket, &servaddr, cstr);
    }
  }
  while(true) {
    ret = recvIQSamples(fd, &centerFrequency, &gain, &nSamples, iq);
