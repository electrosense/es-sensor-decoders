...
static ssize_t recvIQSamples(int fd, uint64_t *centerFrequency, uint32_t * gain, uint32_t *nSamples, int8_t *iq) {
    ...
    *nSamples = le32toh(hdr.nSamples);
    return recv(fd, iq, *nSamples, MSG_WAITALL);
}
static int8_t iq[256 * 1024 * 2];
static int8_t output[DEFAULT_BUF_LENGTH];
...
static void *rtlsdr_thread_fn(void *arg)
{
	...
	ringbuf_t buffer = ringbuf_new(DEFAULT_BUF_LENGTH*2);
	...
	int rest=0;
	long long total_in_samples=0;
	long long total_out_samples=0;
	unsigned i;

	long buffer_length = ctx->both.len_in;
	...
	while(ctx->active) {
		...
		/* Maximum data size is 256*1024 bytes */
		ringbuf_memcpy_into(buffer, iq, nSamples);


		// Add pending buffer info
		while (ringbuf_memcpy_from(output, buffer, DEFAULT_BUF_LENGTH))
		{
			pthread_rwlock_wrlock(&ctx->both.rw);
			for (i = 0; i < DEFAULT_BUF_LENGTH; i++)
				ctx->both.buf[i] = ((int16_t)output[i])-127;
			pthread_rwlock_unlock(&ctx->both.rw);
			safe_cond_signal(&ctx->ready, &ctx->ready_m);
			total_out_samples+=DEFAULT_BUF_LENGTH/2;
		}
	}
	ringbuf_free(&buffer);
	ctx->active = 0;
...
struct rtl_ais_context *rtl_ais_start(struct rtl_ais_config *config)
{
	...
	int ret = init_ais_decoder(config->host, config->port, config->show_levels,
		config->debug_nmea, ctx->stereo.bl_len, config->seconds_for_decoder_stats,
		 config->use_tcp_listener, config->tcp_keep_ais_time);
	if (ret != 0)
	{
